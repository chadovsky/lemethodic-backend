...
class Feedback(Base):
    ...
    # ── F-084 narrative summary (single-sentence diagnostic hero) ──
    # Generated by the extended F-083 rubric prompt as the LAST output
    # field after dimension scoring. Voice: deadpan tutor for English
    # speakers ("B1+, headed to B2. Connectors are holding you back.").
    # Stored in its own column for queryability + cleaner API surface;
    # also retained inside tache_rubric_data blob since the LLM emits
    # both in one pass. Nullable — legacy rows return null and the
    # frontend falls back to the CEFR-band default heading.
