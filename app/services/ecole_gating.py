"""F-053 — L'École gating.

Thin read-side service for any code that needs to check curriculum
progress. The canonical source is the ``user_ecole_progress`` table,
...
- ``next_lesson(user, db)`` — the next unlocked-but-not-completed
  ``EcoleLesson`` row, or the last lesson once everything's done.
...
def ensure_progress_rows(user: User, db: Session) -> list[UserEcoleProgress]:
    """Create the per-user ``UserEcoleProgress`` rows if missing.
    ...
    if not lessons:
        # École hasn't been seeded yet — nothing to bootstrap.
        return []

    existing = {
        row.lesson_id: row
        for row in db.query(UserEcoleProgress)
...
def lessons_completed(user: User, db: Session) -> int:
    ...
    return (
        ...
        .filter(
            ...
            UserEcoleProgress.status == "completed",
        )
        .count()
    )
...
def is_above_a2(user: User, db: Session) -> bool:
    """True iff the user has finished all 16 L'École lessons.
...
def next_lesson(user: User, db: Session) -> Optional[EcoleLesson]:
    ...
    rows = (
        db.query(UserEcoleProgress, EcoleLesson)
        .join(EcoleLesson, UserEcoleProgress.lesson_id == EcoleLesson.id)
        .filter(UserEcoleProgress.user_id == user.id)
        .order_by(EcoleLesson.lesson_number)
        .all()
    )
...
def record_quiz_attempt(
    ...
    progress = (
        ...
        .filter(
            UserEcoleProgress.user_id == user.id,
            UserEcoleProgress.lesson_id == lesson.id,
        )
        .first()
    )
    if progress is None:
        # Lazy-create if we somehow don't have the row yet (e.g. the
        # ecole tables were populated after this user signed up).
        ensure_progress_rows(user, db)
        progress = (
            db.query(UserEcoleProgress)
            .filter(
    ...
    if passed:
        ...
        next_row = (
            ...
            .join(
                EcoleLesson,
                UserEcoleProgress.lesson_id == EcoleLesson.id,
            )
            .filter(
                UserEcoleProgress.user_id == user.id,
                EcoleLesson.lesson_number == (lesson.lesson_number or 0) + 1,
            )
            .first()
        )
